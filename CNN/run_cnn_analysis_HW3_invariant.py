...
from conv_mod_inv import ArtistConvNet as AC
...
def run_all_cnn_analysis():
  ...
  layer1_depth_array = [64]
  layer1_stride_array = [4] 
  layer2_filter_size_array = [15] 
  layer2_depth_array = [64]
  layer2_stride_array = [3] 
  layer3_num_hidden_array = [64] 
  layer4_num_hidden_array = [64] 
  num_training_steps_array = [4*1501] 
  
  # Add max pooling
  pooling_array = [True]
  layer1_pool_filter_size_array = [16]
  layer1_pool_stride_array = [1]
  layer2_pool_filter_size_array = [8]
  layer2_pool_stride_array = [2]

  # Enable dropout and weight decay normalization
  dropout_prob_array = [0.8485] # set to < 1.0 to apply dropout, 1.0 to remove
  weight_penalty_array = [0.001] # set to > 0.0 to apply weight penalty, 0.0 to remove
...
def save_to_mat(output_list,input_info):
  try:
    sio.savemat('cnn_best_results.mat', {'results': output_list,'inputs': input_info})
