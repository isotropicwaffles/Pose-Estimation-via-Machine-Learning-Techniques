function [A,point,Rt]=generate_noisy_input_data(data,std_noise,draw_plot)
%type = 1     cube
%type = 2     cone 
%type = 3      sphere
...
while i<=n
    ...
    Xcam= poseMat(data.Runs(ii).x, data.Runs(ii).y,data.Runs(ii).z, data.Runs(ii).yaw, data.Runs(ii).pitch, data.Runs(ii).roll)*[data.Points(:,1); 1];
    point(i).Xcam(1,1)=Xcam(1);
    point(i).Xcam(2,1)=Xcam(2);
    point(i).Xcam(3,1)=Xcam(3);
    data.Runs(ii).Points.*256+256;
